...
type Props = { pet?: Pet; reminders: CareReminder[]; growthRecords: GrowthRecord[]; onBack: () => void; onSaveGrowth: (record: GrowthRecord) => Promise<void>; onDeleteGrowth: (record: GrowthRecord) => Promise<void>; onExportVetReport: () => void }
const statusLabels = { completed: '已完成', late: '已補做', skipped: '本次略過', scheduled: '預定', recorded: '已有紀錄' }

export default function HealthTimeline({ pet, reminders, growthRecords, onBack, onSaveGrowth, onDeleteGrowth, onExportVetReport }: Props) {
  ...
  return <section className="timeline-page"><header className="timeline-header"><button onClick={onBack}>‹</button><div><span className="eyebrow">LOCAL HEALTH HISTORY</span><h1>{pet?.name || '毛孩'}的健康時間軸</h1><p>生活照護與健康紀錄，都留在這台手機。</p></div></header>
    <button className="timeline-export" onClick={onExportVetReport}>產生獸醫摘要 PDF<span>只整理目前毛孩的健康資料，不含照片與錄音</span></button>
    ...
    {Object.keys(monthGroups).length ? Object.entries(monthGroups).map(([month, items]) => <section className="timeline-month" key={month}><h2>{month}</h2><div className="timeline-list">{items.map((event) => <article key={event.id} onClick={() => {
      if (event.id.startsWith('comparison-')) {
        window.location.hash = '#/visual-comparison'
      } else if (event.id.startsWith('abnormal-')) {
        window.location.hash = '#/event'
      }
    }} style={{ cursor: (event.id.startsWith('comparison-') || event.id.startsWith('abnormal-')) ? 'pointer' : 'default' }}><time><b>{event.date.getDate()}</b><small>{event.date.toLocaleDateString('zh-TW', { weekday: 'short' })}</small></time><i className={event.kind}>{kindIcons[event.kind]}</i><div><span>{kindLabels[event.kind]}・{event.date.toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit', hour12: false })}</span><h3>{event.title}</h3><p>{event.details}</p></div><em className={event.status}>{statusLabels[event.status]}</em></article>)}</div></section>) : <div className="timeline-empty"><i>♡</i><b>還沒有這類紀錄</b><p>完成照護、建立看診或疫苗提醒後，會自動出現在這裡。</p></div>}
  </section>
}
